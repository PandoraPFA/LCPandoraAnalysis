...
public:
   ...
   */
   ...
   float GetHadronicEnergyInLayer(const Event &event, const unsigned int pseudoLayer) const;
    ...
    bool                         m_minimizeUsingClusterEnergy; ///< Whether to minimize the chi2 using true energy or reconstructed energy
...
int main(int argc, char **argv)
{
    try
    {
        ...
        data_file << "True energy used                                   : " << (softwareCompensation.m_minimizeUsingClusterEnergy ? "true" : "false") << std::endl;
        ...
        std::cout << "True energy used                                   : " << (softwareCompensation.m_minimizeUsingClusterEnergy ? "true" : "false") << std::endl;
...
SoftwareCompensation::SoftwareCompensation() :
    ...
    m_minimizeUsingClusterEnergy(false),
...
double SoftwareCompensation::MinuitChi2(const double *params)
{
    ...
    for (unsigned int i = 0 ; i < nEvents ; i++)
    {
        try
        {
          ...
          const double pfoEnergy(m_minimizeUsingClusterEnergy ? event.m_reconstructedEnergy : event.m_trueEnergy);
...
bool ParseCommandLine(int argc, char *argv[], SoftwareCompensation &softwareCompensation)
{
    ...
    while (((c = getopt(argc, argv, "d:e:f:gt:")) != -1) || (argc == 1))
    {
        ...
        case 'f':
            ...
            break;
        case 'g':
            softwareCompensation.m_minimizeUsingClusterEnergy = true;
            break;
        case 't':
        ...
        default:
            std::cout << std::endl << "PandoraPFA_SoftwareCompensation " << std::endl
                      ...
                      << "    -g           (if specified, the minimization will use the cluster energy instead of true energy (mc or beam))" << std::endl
